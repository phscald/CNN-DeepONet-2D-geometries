...
for j in indexes:
    ...
    # sns.lineplot(x=[-.1, -.1], y=[0, 400], color='red', linewidth=1.5, linestyle='--')
        ...
        if aux_groups[groups[i]][inds[i],1] == 5/21:
            ...
            print(aux_groups[groups[i]][inds[i]])
            print(f' MSE CNN: {np.round(mean_squared_error(y_groups[groups[i]][inds[i]], y_pred_CNN),4):.4f},\n MSE DON: {np.round(mean_squared_error(y_groups[groups[i]][inds[i]], y_pred_DON),4):.4f},\n $%oil\_error$ CNN: {np.round(oil_error(y_groups[groups[i]][inds[i]], y_pred_CNN),3):.3f},\n $%oil\_error$ DON: {np.round(oil_error(y_groups[groups[i]][inds[i]], y_pred_DON),3):.3f}')

        elif aux_groups[groups[i]][inds[i],1] == 10/21:
            vinj = np.arange(stop=vinj_10[j]+(vinj_10[j]/49)*.1,step = (vinj_10[j]/49))
            sns.lineplot(x= vinj, y= y_pred_DON, color=colors[1], linestyle='--')
            sns.lineplot(x= vinj, y= y_pred_CNN, color=colors[1], linestyle=':')
            sns.lineplot(x= vinj, y= y_groups[groups[i]][inds[i]], color=colors[1], linestyle='-')
            print(aux_groups[groups[i]][inds[i]])
            print(f' MSE CNN: {np.round(mean_squared_error(y_groups[groups[i]][inds[i]], y_pred_CNN),4):.4f},\n MSE DON: {np.round(mean_squared_error(y_groups[groups[i]][inds[i]], y_pred_DON),4):.4f},\n $%oil\_error$ CNN: {np.round(oil_error(y_groups[groups[i]][inds[i]], y_pred_CNN),3):.3f},\n $%oil\_error$ DON: {np.round(oil_error(y_groups[groups[i]][inds[i]], y_pred_DON),3):.3f}')

        elif aux_groups[groups[i]][inds[i],1] == 20/21:
            vinj = np.arange(stop=vinj_20[j]+(vinj_20[j]/49)*.1,step = (vinj_20[j]/49))
            sns.lineplot(x= vinj, y= y_pred_DON, color=colors[2], linestyle='--')
            sns.lineplot(x= vinj, y= y_pred_CNN, color=colors[2], linestyle=':')
            sns.lineplot(x= vinj, y= y_groups[groups[i]][inds[i]], color=colors[2], linestyle='-')
            print(aux_groups[groups[i]][inds[i]])
            print(f' MSE CNN: {np.round(mean_squared_error(y_groups[groups[i]][inds[i]], y_pred_CNN),4):.4f},\n MSE DON: {np.round(mean_squared_error(y_groups[groups[i]][inds[i]], y_pred_DON),4):.4f},\n $%oil\_error$ CNN: {np.round(oil_error(y_groups[groups[i]][inds[i]], y_pred_CNN),3):.3f},\n $%oil\_error$ DON: {np.round(oil_error(y_groups[groups[i]][inds[i]], y_pred_DON),3):.3f}')
    ...
    sns.lineplot(x= [0], y= [0], color=colors[1], linestyle='-', label=r'$\mu_o/\mu_w=10$')
    sns.lineplot(x= [0], y= [0], color=colors[2], linestyle='-', label=r'$\mu_o/\mu_w=20$')
    ...
    plt.savefig(path+path_img+'predictions'+str(j)+'.eps', format='eps')